...
def step():
    '''Steps the simulation forward in time by dt seconds'''
...
# Determines whether to animate the planets or collect data.
# False to collect data (useful), True to animate (cute).
animate = False
...
# Timestep (seconds). Determines time resolution. Higher timestep -> less accuracy.
dt = 43200

# Initial coordinates for each body. Data was gathered from barycentric ephemerides
# generated by JPL Horizons' web interface in late November 2017.
# Arrays are formatted as follows:
# [x-position (meters from barycenter), y-position (meters from barycenter),
# x-velocity (m/s), y-velocity (m/s), mass (kg)]
...
# Setting state to the initial planetary coordiantes.
state = np.array([sun, mercury, venus, earth, mars, jupiter, saturn, uranus, neptune])

# Store simulation data.  Specifically, each element in the list is a tuple
# containing the days elapsed and the x and y distance of every planet from the sun.
record = []
...
if not animate:
    # Runs the simulation for 1 million years (this will take several days).
    # nsteps is the number of 12 hour timesteps in 1 million years.
    # Decrease nsteps to lower the length of the simulation.
    nsteps = 2*365*1000000
    for j in range(nsteps):
        step()
        timeclocked += dt
        if j % 1000 == 0:
            # At every 1000th step, the number of steps taken is printed in order to
            # measure simulation progress, and the planetary positions are recorded.
            print(j)
            days = (timeclocked*1.15741e-5) # days elapsed
    ...
    # Animate the planets in motion.
    # Setting up figure.
    fig = plt.figure()
    ax = fig.add_subplot(111, aspect='equal')
    fig.patch.set_facecolor('black')

    # Drawing circles to compare against the planets' actual orbits.
    ...
    # Setting bounds (meters).
    plt.axis('off')
    plt.axis([-48e11,48e11,-48e11,48e11])
